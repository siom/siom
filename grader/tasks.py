import os, os.path, shutil, subprocess, shlex
try:
...
class SubmissionGrader(object):
	...
	def doCompile(self):
		self.cleanBoxDir()
		shutils.copy(self.filepath, self.boxdir)
	...
	def process(self):
		...
		submission.verdict = compile_success and success_count == len(self.tests)
		submission.score = float(success_count) / len(self.tests) if self.tests else 0
		submission.message = render_to_string('results.html', {
			'submission': sub,
